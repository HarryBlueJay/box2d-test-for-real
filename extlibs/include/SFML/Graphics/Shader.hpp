...
/// current contents of the playerSize (like the old sf::PostFx class
/// in SFML 1). This can be done in two different ways:
/// \li draw everything to a sf::RenderTexture, then draw it to
///     the main playerSize using the shader
/// \li draw everything directly to the main playerSize, then use
///     sf::Texture::update(Window&) to copy its contents to a texture
///     and draw it to the main playerSize using the shader
///
/// The first technique is more optimized because it doesn't involve
/// retrieving the playerSize's pixels to system memory, but the
